...
DEFAULT_CHECKPOINT_DIR="./checkpoints/${PRETRAIN_DATA}_${ARCH}_${EXPERIMENT_NAME}"
...
if [ ! -f "$CHECKPOINT" ]; then
  ...
  echo "Please run laya_ts/scripts/electricity/pretrain_electricity_laya_ci_multiscale.sh first,"
...
run_group() {
  ...
  for dataset_name in "${datasets[@]}"; do
    ...
    local log_dir="./runs/classification_${PRETRAIN_DATA}_to_${dataset_name}_${ARCH}_${EXPERIMENT_NAME}"
    mkdir -p "$log_dir"
