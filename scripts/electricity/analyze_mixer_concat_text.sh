...
ARCH="laya"
DEFAULT_CHECKPOINT_DIR="${DEFAULT_CHECKPOINT_DIR:-./checkpoints/${PRETRAIN_DATA}_${ARCH}_mixer_concat_text}"
...
TOP_K="${TOP_K:-5}"
SEQ_LEN="${SEQ_LEN:-512}"
OUT_DIR="${OUT_DIR:-./analysis/mixer_structure/electricity_mixer_concat_text}"
DEVICE="${DEVICE:-cuda}"
SEED="${SEED:-42}"
...
if [ ! -f "$CHECKPOINT" ]; then
  ...
  echo "Please run laya_ts/scripts/electricity/pretrain_electricity_laya_mixer_concat_text.sh first,"
