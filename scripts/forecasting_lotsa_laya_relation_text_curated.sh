...
DEFAULT_CHECKPOINT_DIR="./checkpoints/${PRETRAIN_DATA}_${ARCH}_relation_${CHANNEL_METADATA_MODE}"
...
if [ ! -f "$CHECKPOINT" ]; then
  ...
  echo "Please run ./scripts/pretrain_lotsa_laya_relation_text_curated.sh first,"
...
for DATA in "${TARGET_DATASETS[@]}"; do
  ...
  LOG_DIR="./runs/forecasting_${PRETRAIN_DATA}_to_${DATA}_${ARCH}_relation_${CHANNEL_METADATA_MODE}_curated"
  mkdir -p "$LOG_DIR"
