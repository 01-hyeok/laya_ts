...
if [ ! -f "$CHECKPOINT" ]; then
  ...
  echo "Please run laya_ts/scripts/lotsa-all/pretrain_lotsa_laya_mixer_concat_text_stats_joint.sh first,"
...
for DATA in "${TARGET_DATASETS[@]}"; do
  ...
  LOG_DIR="./runs/forecasting_${PRETRAIN_DATA}_to_${DATA}_${ARCH}_mixer_concat_text_stats_joint"
  mkdir -p "$LOG_DIR"
