...
LOTSA_ROOT_DEFAULT="${REPO_ROOT}/../Dataset/LOTSA"
cd "${REPO_ROOT}"
...
LOTSA_DATASET_PATH="${LOTSA_ROOT_DEFAULT}"
...
SAVE_DIR="${SAVE_DIR:-./checkpoints/${RUN_DATA}_${ARCH}_metadata_query_gate_text_stats_joint}"
LOG_DIR="${LOG_DIR:-./runs/pretrain_${RUN_DATA}_${ARCH}_metadata_query_gate_text_stats_joint}"

export LAYA_TS_LOG_TEXT_METADATA_PREVIEW="${LOG_TEXT_METADATA_PREVIEW}"
export LAYA_TS_DEBUG_LOTSA="${DEBUG_LOTSA}"

echo "🚀 Laya-TS Pretraining: ${RUN_DATA} (${ARCH} + metadata_query_gate_text_stats_joint)"
...
echo "🔗 query-gate scale init: ${CHANNEL_MIXER_RELATION_SCALE_INIT}"
echo "📝 log_dir: ${LOG_DIR}"

EXTRA_ARGS=(
  --attention_map_tag metadata_query_gate_text_stats_joint
