...
DEFAULT_CHECKPOINT_DIR="./checkpoints/${PRETRAIN_DATA}_${ARCH}_metadata_query_gate_text_stats_joint"
...
if [ ! -f "$CHECKPOINT" ]; then
  ...
  echo "Please run laya_ts/scripts/lotsa-all/pretrain_lotsa_laya_metadata_query_gate_text_stats_joint.sh first,"
...
run_group() {
  ...
  for dataset_name in "${datasets[@]}"; do
    ...
    local log_dir="./runs/classification_${PRETRAIN_DATA}_to_${dataset_name}_${ARCH}_metadata_query_gate_text_stats_joint"
    mkdir -p "$log_dir"
