...
if [ ! -f "$CHECKPOINT" ]; then
  ...
  echo "Please run laya_ts/scripts/pretrain_lotsa_laya_metadata_query_gate_stats.sh first,"
...
run_group() {
  ...
  for dataset_name in "${datasets[@]}"; do
    ...
    local log_dir="./runs/classification_${PRETRAIN_DATA}_to_${dataset_name}_${ARCH}_metadata_query_gate_stats"
    mkdir -p "$log_dir"
