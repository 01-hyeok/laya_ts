...
CHANNEL_MIXER_RELATION_MODE="${CHANNEL_MIXER_RELATION_MODE:-laya_relation}"
CHANNEL_MIXER_RELATION_SCALE_INIT="${CHANNEL_MIXER_RELATION_SCALE_INIT:-0.01}"
USE_CHANNEL_RELATION_BLOCK="${USE_CHANNEL_RELATION_BLOCK:-0}"
...
DEFAULT_CHECKPOINT_DIR="./checkpoints/${PRETRAIN_DATA}_${ARCH}_laya_relation_text"
...
if [ ! -f "$CHECKPOINT" ]; then
  ...
  echo "Please run laya_ts/scripts/pretrain_electricity_laya_relation.sh first,"
...
run_group() {
  ...
  for dataset_name in "${datasets[@]}"; do
    ...
    local log_dir="./runs/classification_${PRETRAIN_DATA}_to_${dataset_name}_${ARCH}_laya_relation_text"
    mkdir -p "$log_dir"
